...
Perhaps the reason behind this popularity is their simplicity. CA are composed of descentralized interactions of their individual parts (cells), these cells are usually  arranged in a very regular grid and are updated following the same rules for all cells. The rules only take into account the vicinity of a given cell. From this desing specifications one could naively anticipate that a very homogenous state is reached after some iterations of the CA. But this is not always the case, surprinsingly for some simple configurations and rules complex behaviour emerges.
...
Wolfram from extensive simulations of 1-D CA grouped the general behivour of CA in four informally defined classes [@wolfram2002new]. The classification is done by simulation from a variety of lattice initializations to broadly characterize the CA.
...
4. Class 4: This is the most interesting type of behaviour. Patterns that interact in a complex way emerge. These complex patterns are stable for long periods of time. Eventually the complex patterns can settle into a global state like Class 2 behaviour but this can take a vast amount of time. Wolfram has conjectured that many Class 4 
CA are capable of universal computation [@wolfram2002new].  

### Cellular Automata and the modeling of Complex Systems
...
Table 2.1: How different models handle _state_, _space_ and _time_. _C_ stands for continuous and _D_ for discrete. The discrete nature of CA is highlighted.
...
In _Table 2.1_ some assumptions made for some typical models used in the sciences are shown. It could be noted that CA are eminently discrete  
Complexs systems are ...

Modeling of natural or social phenomena using CA is done mainly in bottom up fashion 

....
Predictio are made by strightforward
