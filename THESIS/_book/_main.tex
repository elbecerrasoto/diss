...
Each experiment was run on \emph{\href{https://colab.research.google.com}{Google Colaboratory}} cloud service using their provided GPUs and computing resources. Their run for 1,200,000 epochs each, divided in 400,000 exploration and 800,000 of exploting epochs. Two exploration schemes were used, for some models a linear annealing of \(\epsilon\) from \(1.0\) to \(0.10\) and for others a simple heruristic that moves the helicopter to any fire cell on the neighborhood, choosing randomly when more than one fire cell was present.

The optimization algorithm was Adam (Kingma and Ba \protect\hyperlink{ref-kingma2014adam}{2014}) with default \emph{\href{https://pytorch.org/docs/stable/optim.html}{Pytorch}} values, with the exeption of \emph{learning rate} that was set manually per experiment. Due to the architecture of the \(Q_{network}\) only the weights related to a particular action were updated

Talk about what?
In the 2015 paper they talk about:
...
\caption{\label{tab:3x3}Hyperparameters of 3x3 forest forest environments runs. The return was computed from playing 100,000 steps following the learned policy.}
\centering
\resizebox{\linewidth}{!}{
...
{\centering \includegraphics[width=0.99\linewidth,height=0.99\textheight]{pics/sd_vs_mean_v3} 
...
{\centering \includegraphics[width=0.99\linewidth,height=0.99\textheight]{pics/mean_window_v3_ascending} 
...
{\centering \includegraphics[width=26.24in]{pics/loss_v3_ascending} 
