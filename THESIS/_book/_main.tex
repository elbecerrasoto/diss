...
In this thesis, we consider the advantages of using Cellular Automata as underlying environments for RL tasks. To fully incorporate a CA into the RL framework a Markov Decision Process (MDP) should be defined. So we need to be able to find appropiate sets of \emph{Actions} (\(\mathcal{A}\)), \emph{States} (\(\mathcal{S}\)) and \emph{Rewards} (\(\mathcal{R}\)), also specify an ``Agent'' that interacts with the CA and explicitly or implicitly define the transitions of the MDP \(p(s',r|s,a): \mathcal{S} \times \mathcal{R} \times \mathcal{S} \times \mathcal{A} \rightarrow [0,1]\).
...
\[v_\pi(s) = \mathbb{E}_\pi \bigg[ \sum_{k=0}^{\infty} \gamma^k R_{t+1+k} \bigg| S_t = s \bigg], \quad \forall s \in \mathcal{S}\]
...
\[ q_\pi(s,a) = \mathbb{E}_\pi \bigg[ \sum_{k=0}^{\infty} \gamma^k R_{t+1+k} \bigg| S_t = s, A_t = a \bigg], \quad \forall s \in \mathcal{S}, a \in \mathcal{A}\]
...
\[q_\pi(s,a) = \sum_{s',r} p(s',r|s,a) \big[ r + \gamma \sum_{a'} \pi(s'|a')\ q_\pi(s',a') \big], \quad \forall s \in \mathcal{S}, a \in \mathcal{A} \]
...
\[q_*(s,a) \doteq \underset {\pi} {\max}\ q_\pi(s,a), \quad \forall s \in \mathcal{S}, a \in \mathcal{A}\]
...
The above equalities are also present on they \emph{action-value} \(q_*(s,a)\) form:

\[q_*(s,a) = \mathbb{E} \bigg[ R_{t+1} + \gamma\ \underset{a'} \max\ q_*(S_{t+1}, a')\ \bigg|\ S_t = s, A_t=a \bigg]\]
...
The Arcade Learning Environment (ALE) was introduced in 2013 (Bellemare et al. \protect\hyperlink{ref-bellemare2013arcade}{2013}). It was quickly adopted as benchmark by the RL community. ALE consits on a wrapper around the \href{https://stella-emu.github.io/}{Stella emulator}\footnote{\url{https://stella-emu.github.io/}} for the Atari 2600 console. This wrapper provides an interface to more than 50 Atari games. ALE was used by the Google's DeepMind team in the DQN paper (Mnih et al. \protect\hyperlink{ref-mnih2015human}{2015}), arguibly setting the current trend of deep reinforcement learning.

The desirable characteristics of Atari games for RL evaluation are their variety of tasks, their short episodic nature and scores that can be directly interpreted as rewards. The agents interact with ALE thorugh a set of 18 actions derived from 6 simple actions, namely four directions (up, down, left, right) and fire and NO-OP actions. The final 18 actions are combinations of the basic actions. The state information is raw pixel data with dimensions \(210 \times 160\), with each pixel on 7-bit color value (0-127). An important parameter
